...
void PrintMatrix1(int[,] matrix1)
{
    for (int i = 0; i < matrix1.GetLength(0); i++)
    {
        ...
        for (int j = 0; j < matrix1.GetLength(1); j++)
        {
            if (j < matrix1.GetLength(1) - 1) System.Console.Write($"{matrix1[i, j],4},");
...
{ int[,] matrix2 = new int[array.GetLength(0), array1.GetLength(1)];
 if (array.GetLength(1) != array1.GetLength(0))  Console.WriteLine("нет решения");
 else
...
void PrintMatrix2(int[,] matrix2)
{
    for (int i = 0; i < matrix2.GetLength(0); i++)
    {
        ...
        {
            if (j < matrix2.GetLength(1) - 1) System.Console.Write($"{matrix2[i, j],4},");
...
int[,] array1 = CreateMtrixRndInt1(3, 3, 1, 10);
PrintMatrix1(array1);
Console.WriteLine();
